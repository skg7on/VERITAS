...
// --- Reloaded assembly ------------------------------------------------------
//
// The orchestrator releases a component's payload once the result has been
// stored, so assembly reloads it from the content-addressed object instead of
// reading it out of memory. `ReloadedAssemblyMatchesRetainedAssembly` is what
// makes that substitution sound: assembling a run whose payloads have been
// released, refilled through the loader, must produce exactly the batch that
// assembling the same run's in-memory payloads produces -- the same rows in the
// same order, and therefore the same content-addressed batch id.
//
// The two sides are built independently. `retained` is assembled from payloads
// that were never stored; `reloaded` is assembled from a copy of the same run
// whose payloads were released and are refilled from the store. A defect in
// either the serialization round trip or the loader path therefore shows up as a
// difference instead of agreeing with itself.

// A run whose components survive a store-and-reload: the canonical hashes are
// computed from the rows, which is what `LoadReusableComponent` revalidates
// against, and every witness carries the empty `derivation_key` the canonicalizer
// emits. Two components derive the same global flow, so the duplicate-fact
// ownership pass and its dropped witness edge are exercised too.
wpa::WpaRunResult StorableRun() {
  const SemanticRow shared_flow = GlobalFlow("r", "p");
  const SemanticRow flow_root_a = ParameterFlow("cs:a", "r", "p");
  const SemanticRow flow_root_b = ParameterFlow("cs:b", "r", "p");
  const SemanticRow reachable = Reachable("f", "g");
  const SemanticRow call_root = DirectCall("f", "g");

  const auto component = [](core::StableId scc, wpa::WpaComponentKind kind,
                            std::vector<SemanticRow> rows,
                            std::vector<WitnessEdge> edges,
                            std::vector<std::string> diagnostics) {
    wpa::WpaComponentCompletion completion;
    completion.key = wpa::WpaComponentKey{scc, kind};
    completion.result.scc_id = scc;
    completion.result.component = kind;
    completion.result.logical_input_hash = "logical";
    for (auto &row : rows) {
      completion.result.facts.push_back(MakeFact(row).value());
    }
    completion.result.witnesses = std::move(edges);
    completion.result.diagnostics = std::move(diagnostics);
    const auto hashes = ComputeCanonicalResultHashes(completion.result.facts,
                                                     completion.result.witnesses);
    completion.result.fixpoint_hash = hashes.fixpoint_hash;
    completion.result.external_hash = hashes.external_hash;
    return completion;
  };

  const auto completion_a = component(
      FunctionId("scc:a"), wpa::WpaComponentKind::kFlow, {shared_flow},
      {Edge(shared_flow, kFlowParameter, flow_root_a, 0)},
      {"flow component a diagnostic"});
  const auto completion_b = component(
      FunctionId("scc:b"), wpa::WpaComponentKind::kFlow, {shared_flow},
      {Edge(shared_flow, kFlowParameter, flow_root_b, 0)}, {});
  const auto completion_c =
      component(FunctionId("scc:c"), wpa::WpaComponentKind::kReachability,
                {reachable}, {Edge(reachable, kDirect, call_root, 0)}, {});

  const auto root = [](const SemanticRow &row) { return MakeFact(row).value(); };
  wpa::WpaRunResult run;
  run.run = TestRun();
  run.expected_components = {completion_a.key, completion_b.key, completion_c.key};
  run.completed_components = {completion_a, completion_b, completion_c};
  run.rooted_input_fact_ids = {root(flow_root_a).fact_id, root(flow_root_b).fact_id,
                               root(call_root).fact_id};
  run.rooted_input_facts = {RootedInputFact{.fact = root(flow_root_a)},
                            RootedInputFact{.fact = root(flow_root_b)},
                            RootedInputFact{.fact = root(call_root)}};
  return run;
}

TEST(AnalysisFactBusTest, ReloadedAssemblyMatchesRetainedAssembly) {
  const auto db = TempDbPath();
  auto repo = wpa::WpaRunRepository::Open(db);
  ASSERT_TRUE(repo.ok()) << repo.status().message();
  const wpa::WpaRunResult run = StorableRun();
  ASSERT_TRUE(repo->BeginRun(run.run).ok());

  // Store the run, recording each component's object key as the orchestrator
  // does. The payload the store wrote is the payload the batch id covers.
  wpa::WpaRunResult stored = run;
  for (auto &completion : stored.completed_components) {
    auto written =
        repo->StoreSuccessfulComponent(run.run, completion.key, completion.result);
    ASSERT_TRUE(written.ok()) << written.status().message();
    completion.result_object_key = written->result_object_key;
    ASSERT_FALSE(completion.result_object_key.empty());
  }
  ASSERT_TRUE(repo->FlushComponentCache().ok());

  // The retained side: assembly reading the in-memory payloads. This is the
  // code path that has always assembled the batch.
  const AnalysisFactBatch retained = MakeAnalysisFactBatch(stored);
  ASSERT_FALSE(retained.facts.empty());
  ASSERT_FALSE(retained.witnesses.empty());
  ASSERT_EQ(retained.facts.size(), 2u);
  ASSERT_EQ(retained.witnesses.size(), 2u);

  // The released side: the same run with every payload dropped, exactly as the
  // orchestrator leaves it, refilled through the loader during assembly.
  wpa::WpaRunResult released = stored;
  for (auto &completion : released.completed_components) {
  ...
  const AnalysisRunManifest manifest = released.run;
  const ComponentReloader reload = [&repo, manifest](
                                       const wpa::WpaComponentKey &key) {
    return repo->ReloadStoredComponent(manifest, key);
  };
  auto reloaded = MakeAnalysisFactBatch(std::move(released), reload);
  ASSERT_TRUE(reloaded.ok()) << reloaded.status().message();

  EXPECT_EQ(reloaded->facts, retained.facts);
  EXPECT_EQ(reloaded->witnesses, retained.witnesses);
  EXPECT_EQ(reloaded->diagnostics, retained.diagnostics);
  EXPECT_EQ(reloaded->expected_components, retained.expected_components);
  EXPECT_EQ(reloaded->rooted_input_fact_ids, retained.rooted_input_fact_ids);
  ASSERT_EQ(reloaded->completed_components.size(),
            retained.completed_components.size());
  for (std::size_t i = 0; i < retained.completed_components.size(); ++i) {
    const auto &expected = retained.completed_components[i];
    const auto &actual = reloaded->completed_components[i];
    EXPECT_EQ(actual.key, expected.key);
    EXPECT_EQ(actual.result_object_key, expected.result_object_key);
    EXPECT_EQ(actual.result.logical_input_hash, expected.result.logical_input_hash);
    EXPECT_EQ(actual.result.fixpoint_hash, expected.result.fixpoint_hash);
    EXPECT_EQ(actual.result.external_hash, expected.result.external_hash);
  }
  EXPECT_EQ(reloaded->batch_id, retained.batch_id);
