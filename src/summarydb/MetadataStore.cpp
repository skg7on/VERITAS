...
Status MetadataStore::ApplySchema() {
  // The V1 and V2 schemas are embedded at build time from schema/v1.sql and
  // schema/v2.sql (see schema_v*.h, generated by configure_file), so this is
  // the single source of truth and ApplySchema has no runtime file-path
  // dependency. Both are idempotent and applied in order.
  Status status = ExecuteSQL(db_, kV1SchemaSql);
  if (!status.ok()) {
    return status;
  }
  return ExecuteSQL(db_, kV2SchemaSql);
}
