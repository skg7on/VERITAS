...
v1::EpistemicState AliasEpistemic(const std::string &relationship) {
  if (relationship == "MUST_ALIAS")
    return v1::EPISTEMIC_STATE_MUST;
  if (relationship == "NO_ALIAS")
    return v1::EPISTEMIC_STATE_MUST_NOT;
  if (relationship == "UNKNOWN_ALIAS")
...
OwningFunctionVariant(std::string_view value_name,
...
bool IsOwnedBy(std::string_view value_name, std::string_view function_id,
  ...
  if (owner.has_value())
    return *owner == function_id;
  // Preserve compatibility with pre-M8 synthetic facts that embedded the
  // function-variant ID directly in their diagnostic value name.
  return value_name.find(function_id) != std::string_view::npos;
}

} // namespace

std::vector<v1::FunctionSummary>
MergeSvfFacts(std::vector<v1::FunctionSummary> drafts, const SvfFacts &facts,
              const ::veritas::analysis::llvm::OriginMap &origin_map) {
  // SVF value names are function-qualified LLVM names. Resolve the prefix
  // through OriginMap before comparing it with the stable identity in each
  // draft; hashed function IDs are intentionally unrelated to display names.
  for (auto &draft : drafts) {
    const std::string &function_id = draft.identity().function_variant_id();
    auto owns = [&](const std::string &name) {
      return IsOwnedBy(name, function_id, origin_map);
    };

    for (const auto &flow : facts.value_flows) {
      if (!owns(flow.source.name) && !owns(flow.destination.name)) {
        continue;
      }
    ...
    for (const auto &alias : facts.aliases) {
      if (!owns(alias.left.name) && !owns(alias.right.name))
        continue;
    ...
    for (const auto &effect : facts.refined_memory_effects) {
      if (!owns(effect.memory.name) && !owns(effect.operation.name)) {
        continue;
      }
