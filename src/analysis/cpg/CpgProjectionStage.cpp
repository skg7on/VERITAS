...
core::StableId MakeId(core::IdKind kind, std::string_view value) {
  return core::MakeStableId(kind, std::as_bytes(std::span(value.data(), value.size())));
}
...
core::StableId EdgeId(EdgeKind kind, const core::StableId& source,
  ...
  std::string content = std::to_string(static_cast<int>(kind)) + core::ToString(source) +
  ...
  for (const auto& record : support) {
    content += core::ToString(record.function_summary_id) + record.provenance_ref;
  }
  return MakeId(core::IdKind::kCpgEdge, content);
}

Status AddSemanticEdge(ThinCpg* graph, EdgeKind kind, const core::StableId& source,
                       const core::StableId& target,
                       std::optional<AliasState> alias_state,
...
StatusOr<ThinCpg> BuildThinCpg(const CpgProjectionInput& input) {
  ...
  for (const auto& function : *module) {
    ...
    core::StableId node_id = MakeId(core::IdKind::kFunctionVariant, symbol);
    function_ids.emplace(symbol, node_id);
    auto status =
        graph.AddNode(CpgNode{node_id, NodeKind::kFunction, std::move(symbol)});
  ...
  for (const auto& summary : input.completed_summaries) {
    ...
    if (!summary_id_result.ok()) return summary_id_result.status();
    ...
    auto add_summary_node = graph.AddNode(
        CpgNode{summary_id, NodeKind::kSummary, fn});
    if (!add_summary_node.ok()) return add_summary_node;
    ...
    {
      auto status = AddSemanticEdge(&graph, EdgeKind::kSummarizedBy, fn_it->second,
                                    summary_id, std::nullopt,
    ...
    for (const auto& call : summary.calls()) {
      auto callee_it = function_ids.find(call.callee_symbol());
      if (callee_it != function_ids.end()) {
        ...
        auto add_unknown =
            graph.AddNode(CpgNode{unknown, NodeKind::kUnknown, call.callee_symbol()});
        if (!add_unknown.ok()) return add_unknown;
        auto status = AddSemanticEdge(
    ...
    for (const auto& effect : summary.memory_effects()) {
      ...
      auto add_memory =
          graph.AddNode(CpgNode{memory, NodeKind::kMemoryObject, effect.location()});
      if (!add_memory.ok()) return add_memory;
      const EdgeKind kind = effect.kind() == v1::EFFECT_KIND_WRITE
                                ? EdgeKind::kWrites
                                : EdgeKind::kReads;
      auto status = AddSemanticEdge(
          &graph, kind, fn_it->second, memory, std::nullopt,
    ...
    for (const auto& flow : summary.value_flows()) {
      core::StableId source = MakeId(core::IdKind::kValueRef, fn + ":" + flow.source());
      core::StableId sink = MakeId(core::IdKind::kValueRef, fn + ":" + flow.sink());
      auto add_src = graph.AddNode(CpgNode{source, NodeKind::kParameter, flow.source()});
      auto add_sink = graph.AddNode(CpgNode{sink, NodeKind::kParameter, flow.sink()});
      if (!add_src.ok()) return add_src;
      if (!add_sink.ok()) return add_sink;
      auto status = AddSemanticEdge(
    ...
    for (const auto& alias : summary.alias_facts()) {
      core::StableId left = MakeId(core::IdKind::kMemoryRef, fn + ":" + alias.location_a());
      core::StableId right = MakeId(core::IdKind::kMemoryRef, fn + ":" + alias.location_b());
      auto add_left = graph.AddNode(CpgNode{left, NodeKind::kMemoryObject, alias.location_a()});
      auto add_right = graph.AddNode(CpgNode{right, NodeKind::kMemoryObject, alias.location_b()});
      if (!add_left.ok()) return add_left;
      if (!add_right.ok()) return add_right;
      auto status = AddSemanticEdge(
          &graph, EdgeKind::kAliases, left, right, ToAliasState(alias.epistemic()),
    ...
    for (const auto& unknown : summary.unknowns()) {
      core::StableId unknown_id =
          MakeId(core::IdKind::kUnknownNode, unknown.scope() + ":" + unknown.reason());
      auto add_unknown =
          graph.AddNode(CpgNode{unknown_id, NodeKind::kUnknown, unknown.kind()});
      if (!add_unknown.ok()) return add_unknown;
      auto status = AddSemanticEdge(
