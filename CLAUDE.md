...
**Analysis Stack (V1):** C++20, CMake 3.23+, LLVM/Clang 22+ libraries, required pinned SVF (`third_party/SVF@18fb5650…`), Z3, Protobuf, RocksDB, SQLite, vendored pinned Soufflé (`third_party/Souffle@5682a9f…`, tag 2.5), GoogleTest. SVF is mandatory for L1 Andersen pointer analysis; no `VERITAS_ENABLE_SVF` toggle exists. The vendored Soufflé build is the mandatory production WPA engine (M8R.4).
